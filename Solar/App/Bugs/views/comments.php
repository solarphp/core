...
<h3><?php $this->eprint($this->locale('USER_COMMENTS')) ?></h3>
		...
		<tr>
			<td style="border: 2px solid #bcd; background: #eee;">
				<p style="background: #bcd; padding: 4px; margin: 0px;"><strong>[ <?php
						...
						if ($val['user_id']) {
							$this->eprint(' | ' . $val['user_id']);
						} elseif ($val['email']) {
							$this->eprint(' | ' . $val['email']);
						}
				?> ]</strong></p>
				<pre style="padding: 8px;"><?php $this->eprint(wordwrap($val['body'])) ?></pre>
			</td>
		</tr>
		<tr><td><br /></td></tr>
	<?php endforeach ?>
...
<?php else: ?>
	...
	<p><?php $this->eprint($this->locale('NO_COMMENTS')) ?></p>
